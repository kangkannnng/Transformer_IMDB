...
# %% [markdown]
# ## 消融实验版本
# 尝试移除模型中的多头自注意力机制，只使用单头注意力机制来建模输入序列之间的关系。对比移除多头自注意力机制后的模型性能与原始模型的性能差异，以评估多头自注意力机制的影响。
...
plt.xlabel("Epoch")
plt.plot(x, y)
plt.savefig('single_train_metrics.jpg')
plt.show()
...
plt.xlabel("Epoch")
plt.plot(x, y)
plt.savefig('single_val_metrics.jpg')
plt.show()
